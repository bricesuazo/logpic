...
const enforceEmployeeIsAuthed = t.middleware(({ ctx, next }) => {
  if (
    !ctx.session ||
    !ctx.session.user ||
 ...
 */
export const employeeProcedure = t.procedure.use(enforceEmployeeIsAuthed);
export const hrProcedure = t.procedure.use(enforceHRIsAuthed);
